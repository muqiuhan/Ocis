...
type Connection (socket : Socket, db : OcisDB, connectionId : string) =
            ...
            if headerBytesRead < Protocol.HEADER_SIZE then
                ...
                | Some header ->
                    if Protocol.IsValidPacketSize header.TotalPacketLength then
                        ...
                        Logger.Warn $"Connection {connectionId} received packet with invalid size: {header.TotalPacketLength}"
                        return None
                | None ->
    ...
    member private this.ReadExactAsync (stream : NetworkStream, buffer : byte array, count : int) = async {
        return! this.ReadExactAsync (stream, buffer, count, 0)
    }

    /// Ensure reading specified number of bytes (with offset)
    member private this.ReadExactAsync (stream : NetworkStream, buffer : byte array, count : int, offset : int) = async {
        ...
        while shouldContinue
            ...
            let! bytesRead =
                stream.ReadAsync (buffer, currentOffset, count - totalRead, cancellationTokenSource.Token)
                |> Async.AwaitTask
            if bytesRead = 0 then
                // Connection closed
    ...
    member private this.SendResponseAsync (stream : NetworkStream, response : ResponsePacket) = async {
        try
...
module ConnectionManager =
    ...
    let startConnection (connection : Connection) = async {
