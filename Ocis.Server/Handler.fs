...
module RequestHandler =
    ...
    async {
      try
        ...
        | CommandType.Set ->
          ...
          | Some value -> return! handleSet dispatcher request value
          | None ->
            Logger.Error "SET command missing value"
            return Protocol.CreateErrorResponse "SET command requires a value"
        | CommandType.Get -> return! handleGet dispatcher request
        | CommandType.Delete -> return! handleDelete dispatcher request
        | _ -> return handleUnknown request
      with ex ->
        let errorMsg =
  ...
  let ValidateRequest (request : RequestPacket) : Result<unit, string> =
    ...
    // Validate key and value actual length matches declared length
      ...
      match request.Value with
      | Some value when value.Length <> request.ValueLength ->
