...
module OcisServerOptions =
  ...
  let ToConfigurationValues (config : OcisConfig) =
    ...
    values.Add (
      $"{SectionName}:L0CompactionThreshold",
      string options.L0CompactionThreshold
    )

    values.Add (
      $"{SectionName}:LevelSizeMultiplier",
      string options.LevelSizeMultiplier
    )
    ...
    values.Add (
      $"{SectionName}:GroupCommitWindowMs",
      string options.GroupCommitWindowMs
    )

    values.Add (
      $"{SectionName}:GroupCommitBatchSize",
      string options.GroupCommitBatchSize
    )

    values.Add (
      $"{SectionName}:DbQueueCapacity",
      string options.DbQueueCapacity
    )

    values.Add (
      $"{SectionName}:CheckpointMinIntervalMs",
      string options.CheckpointMinIntervalMs
    )
...
type OcisHostedService (options : IOptions<OcisServerOptions>) =
  ...
  interface IHostedService with
    member _.StartAsync (cancellationToken : CancellationToken) =
      match
        Ocis.TryCreateRuntime (options.Value |> OcisServerOptions.ToConfig)
      with
      | Ok createdRuntime ->
