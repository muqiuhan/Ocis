...
type LibraryTests() =
  ...
  member this.TestCreateRequest() =
    ...
    let getRequest =
      Ocis.Protocol.CreateRequest CommandType.Get key None
    ...
    let deleteRequest =
      Ocis.Protocol.CreateRequest CommandType.Delete key None
  ...
  member this.TestSerializeRequest() =
    ...
    let parsedMagicNumber =
      BitConverter.ToUInt32(serializedBytes, 0)
    let parsedVersion = serializedBytes.[4]
    let parsedCommandType =
      enum<CommandType>(int serializedBytes.[5])
    let parsedTotalPacketLength =
      BitConverter.ToInt32(serializedBytes, 6)
    let parsedKeyLength = BitConverter.ToInt32(serializedBytes, 10)
    let parsedValueLength =
      BitConverter.ToInt32(serializedBytes, 14)
    ...
    let extractedValue =
      Array.sub serializedBytes valueStart value.Length
    ...
    let getRequest =
      Ocis.Protocol.CreateRequest CommandType.Get key None
    let serializedGetBytes =
      Ocis.Protocol.SerializeRequest getRequest
  ...
  member this.TestRoundTripSerialization() =
    ...
    let request =
      ProtocolHelper.createSetRequest originalKey originalValue
    ...
    let extractedKey =
      Array.sub serializedBytes HEADER_SIZE parsedKeyLen
