...
type LibraryTests () =
    ...
    member this.TestCreateRequest () =
        ...
        let setRequest = Ocis.Protocol.CreateRequest CommandType.Set key (Some value)
        ...
        Assert.That (setRequest.TotalPacketLength, Is.EqualTo (HEADER_SIZE + key.Length + value.Length))
        ...
        Assert.That (getRequest.TotalPacketLength, Is.EqualTo (HEADER_SIZE + key.Length))
        ...
        Assert.That (deleteRequest.TotalPacketLength, Is.EqualTo (HEADER_SIZE + key.Length))
    ...
    member this.TestSerializeRequest () =
        ...
        Assert.That (serializedBytes.Length, Is.EqualTo (setRequest.TotalPacketLength))
        ...
        Assert.That (parsedTotalPacketLength, Is.EqualTo (setRequest.TotalPacketLength))
        ...
        Assert.That (serializedGetBytes.Length, Is.EqualTo (getRequest.TotalPacketLength))
        Assert.That (serializedGetBytes.Length, Is.EqualTo (HEADER_SIZE + key.Length))
    ...
    member this.TestDeserializeResponse () =
        ...
        | ParseError msg -> Assert.Fail (sprintf "Failed to parse valid response: %s" msg)
        ...
        | ParseError msg -> Assert.Fail (sprintf "Failed to parse valid error response: %s" msg)
        | InsufficientData -> Assert.Fail ("Insufficient data for valid error response")
        ...
        | ParseSuccess response ->
            ...
            Assert.That (response.Value, Is.EqualTo (None))
            Assert.That (response.ErrorMessage, Is.EqualTo (None))
        | ParseError msg -> Assert.Fail (sprintf "Failed to parse valid not found response: %s" msg)
        | InsufficientData -> Assert.Fail ("Insufficient data for valid not found response")
    ...
    member this.TestProtocolHelper () =
        ...
        Assert.That (setRequest.CommandType, Is.EqualTo (CommandType.Set))
        Assert.That (ProtocolHelper.bytesToString setRequest.Key, Is.EqualTo ("key1"))
        Assert.That (setRequest.Value.IsSome, Is.True)
        Assert.That (ProtocolHelper.bytesToString setRequest.Value.Value, Is.EqualTo ("value1"))

        // test create GET request
        let getRequest = ProtocolHelper.createGetRequest "key2"
        Assert.That (getRequest.CommandType, Is.EqualTo (CommandType.Get))
        Assert.That (ProtocolHelper.bytesToString getRequest.Key, Is.EqualTo ("key2"))
        Assert.That (getRequest.Value, Is.EqualTo (None))

        // test create DELETE request
        let deleteRequest = ProtocolHelper.createDeleteRequest "key3"
        Assert.That (deleteRequest.CommandType, Is.EqualTo (CommandType.Delete))
        Assert.That (ProtocolHelper.bytesToString deleteRequest.Key, Is.EqualTo ("key3"))
        Assert.That (deleteRequest.Value, Is.EqualTo (None))
    ...
    member this.TestRoundTripSerialization () =
        ...
        let extractedValue = Array.sub serializedBytes (HEADER_SIZE + parsedKeyLen) parsedValueLen

        Assert.That (ProtocolHelper.bytesToString extractedKey, Is.EqualTo (originalKey))
        Assert.That (ProtocolHelper.bytesToString extractedValue, Is.EqualTo (originalValue))
