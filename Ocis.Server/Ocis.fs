    ...
    let rec waitForServerStartup
      ...
      task {
        ...
        if runningTask.IsFaulted then
          ...
          return
            raise (TaskCanceledException ("Server startup was canceled") :> exn)
      ...
      task {
        ...
        | Some runningTask ->
          let! completedTask =
            Task.WhenAny (
              runningTask,
              Task.Delay (TimeSpan.FromSeconds 10.0, cancellationToken)
            )
  ...
  let TryCreateRuntime (config : OcisConfig) : Result<OcisRuntime, string> =
    ...
    | Ok () ->
      ...
      | Ok db ->
        ...
        try
          let dispatcher =
            new OcisDbDispatcher (db, config.DbQueueCapacity)
