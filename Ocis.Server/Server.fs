...
type TcpServer(config: ServerConfig, dispatcher: OcisDbDispatcher) =
  ...
  let activeConnections =
    ConcurrentDictionary<string, Connection>()
  ...
  member this.StartAsync() = async {
    try
      ...
      let endpoint =
        IPEndPoint(IPAddress.Parse config.Host, config.Port)
  ...
  member private this.AcceptConnectionsAsync(listener: TcpListener) = async {
      ...
      while not cancellationTokenSource.Token.IsCancellationRequested
        ...
        try
          ...
          if activeConnections.Count >= config.MaxConnections then
            ...
            let connection =
              ConnectionManager.createConnection tcpClient.Client dispatcher
            ...
            this.HandleConnectionAsync connection
            |> Async.Start
        ...
        with ex ->
          ...
          if shuttingDown then
            ...
            let retryDelayMs =
              computeBoundedRetryDelayMs transientAcceptFailureCount 50 1000
  ...
  member private this.HandleConnectionAsync(connection: Connection) = async {
    try
      try
...
module ServerManager =
  ...
  let runServer (server: TcpServer) (cancellationToken: CancellationToken) = async {
    try
      // Start server
