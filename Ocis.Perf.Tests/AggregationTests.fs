...
type AggregationTests () =
  ...
  member _.AggregateComputesMedianMinMaxAndCv () =
    ...
    Assert.That (
      summary.ThroughputMedianOpsPerSec,
      Is.EqualTo(100.0).Within (0.0001)
    )

    Assert.That (
      summary.ThroughputMinOpsPerSec,
      Is.EqualTo(90.0).Within (0.0001)
    )

    Assert.That (
      summary.ThroughputMaxOpsPerSec,
      Is.EqualTo(110.0).Within (0.0001)
    )

    Assert.That (
      summary.ThroughputCoefficientOfVariation,
      Is.EqualTo(0.08164966).Within (0.000001)
    )
