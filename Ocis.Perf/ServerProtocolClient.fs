...
type ServerProtocolClient (host : string, port : int) =
  ...
  let readExact (count : int) =
    ...
    while totalRead < count do
      ...
      if read = 0 then
        raise (
          EndOfStreamException ("Connection closed while reading response")
        )
  ...
  let sendRequest (request : RequestPacket) : ResponsePacket =
    ...
    let status =
      LanguagePrimitives.EnumOfValue<byte, StatusCode> (
    ...
    let value =
      if valueLength > 0 then
        Some (bodyReader.ReadBytes valueLength)
      else
        None
    ...
    let errorMessage =
      if errLength > 0 then
        Some (
          System.Text.Encoding.UTF8.GetString (bodyReader.ReadBytes errLength)
        )
      else
        None
