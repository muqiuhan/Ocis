...
module ReportWriter =
    let private formatFloat (value: float) = value.ToString("0.###", CultureInfo.InvariantCulture)

    let writeWithSuffix (config: BenchmarkConfig) (summary: RunSummary) (fileSuffix: string option) =
        Directory.CreateDirectory(config.OutputDir) |> ignore

        let timestamp = DateTime.UtcNow.ToString("yyyyMMdd-HHmmss", CultureInfo.InvariantCulture)

        let filePrefix =
            let basePrefix = $"{summary.Target}-{summary.Operation}-{config.OutputTag}-{timestamp}"

            match fileSuffix with
            | Some suffix when not (String.IsNullOrWhiteSpace suffix) -> $"{basePrefix}-{suffix}"
            | _ -> basePrefix

        let jsonPath = Path.Combine(config.OutputDir, $"{filePrefix}.json")
        let csvPath = Path.Combine(config.OutputDir, $"{filePrefix}.csv")
        ...
        File.WriteAllText(csvPath, csvHeader + Environment.NewLine + csvLine + Environment.NewLine, Encoding.UTF8)
        jsonPath, csvPath

    let write (config: BenchmarkConfig) (summary: RunSummary) = writeWithSuffix config summary None

    let writeSummary (config: BenchmarkConfig) (summary: AggregateSummary) =
        Directory.CreateDirectory(config.OutputDir) |> ignore

        let timestamp = DateTime.UtcNow.ToString("yyyyMMdd-HHmmss", CultureInfo.InvariantCulture)
        let summaryPath = Path.Combine(config.OutputDir, $"aggregate-{config.OutputTag}-{timestamp}.json")
