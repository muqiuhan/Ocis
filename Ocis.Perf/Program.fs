...
let private parseInt (value: string) = Int32.Parse(value, CultureInfo.InvariantCulture)
...
let private buildConfig (args: string array) =
    if args.Length = 0 then
        failwith "Usage: dotnet run --project Ocis.Perf -- <engine|server> [--key value ...]"

    let kv = toMap args
    let target = parseTarget args[0]
    let workersDefault = if target = Engine then "1" else "8"
    let dataDirDefault = Path.Combine("BenchmarkDotNet.Artifacts", "throughput-data")
    let outputDirDefault = Path.Combine("BenchmarkDotNet.Artifacts", "results", "throughput")
    ...
    { Target = target
      ...
      AllowUnsafeEngineConcurrency =
        parseBool "--allow-unsafe-engine-concurrency" (getOrDefault kv "--allow-unsafe-engine-concurrency" "false")
    ...
    try
        ...
        let runResults =
            [ for runIndex in 1 .. config.RepeatCount do
                  ...
                  let jsonPath, csvPath = ReportWriter.writeWithSuffix config runSummary fileSuffix
                  printfn "Run %d/%d throughput(ops/s): %.3f" runIndex config.RepeatCount runSummary.ThroughputOpsPerSec
                  printfn "Run %d JSON: %s" runIndex jsonPath
                  printfn "Run %d CSV: %s" runIndex csvPath
                  runSummary, jsonPath, csvPath ]

        let runSummaries = runResults |> List.map (fun (summary, _, _) -> summary)

        let aggregate = Aggregation.summarize runSummaries
        let summaryPath = ReportWriter.writeSummary config aggregate

        let primarySummary, primaryJsonPath, primaryCsvPath = runResults[0]
        ...
        if config.RepeatCount = 1 then
            ...
            printfn "Median throughput(ops/s): %.3f" aggregate.ThroughputMedianOpsPerSec
            printfn "Throughput min/max(ops/s): %.3f / %.3f" aggregate.ThroughputMinOpsPerSec aggregate.ThroughputMaxOpsPerSec
