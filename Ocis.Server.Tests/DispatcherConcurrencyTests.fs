...
type DispatcherConcurrencyTests() =
  ...
  let testDbDir =
    Path.Combine(
      Path.GetTempPath(),
      $"ocis_dispatcher_tests_{Guid.NewGuid():N}"
    )
    ...
    let operations =
      [|for i in 1..300 ->
          async {
            ...
            | _ -> return! dispatcher.DispatchDelete key
          } |]
  ...
  member _.WrittenKeysAreRetrievableAfterConcurrentDispatch() =
    ...
    let writes =
      [|for i in 1..200 ->
          ...
          async {
            let! setResult =
              dispatcher.DispatchSet(toBytes keyText, toBytes valueText)
            return keyText, valueText, setResult
          } |]

    let writeResults =
      writes |> Async.Parallel |> Async.RunSynchronously
    Assert.That(
      writeResults
      |> Array.forall(fun (_, _, result) -> Result.isOk result),
      Is.True
    )

    let reads =
      writeResults
      |> Array.map(fun (keyText, valueText, _) -> async {
        let! getResult = dispatcher.DispatchGet(toBytes keyText)
        return valueText, getResult
      })

    let readResults =
      reads |> Async.Parallel |> Async.RunSynchronously

    readResults
    |> Array.iter(fun (expectedValue, getResult) ->
      match getResult with
      | Ok(Some bytes) ->
        Assert.That(Encoding.UTF8.GetString bytes, Is.EqualTo expectedValue)
      | Ok None ->
        Assert.Fail $"Expected value '{expectedValue}' but key was missing"
  ...
  member _.DispatchAfterStopReturnsControlledError() =
    ...
    let result =
      dispatcher.DispatchGet(toBytes "key-after-stop")
      |> Async.RunSynchronously

    match result with
    | Error msg ->
      Assert.That(msg, Is.EqualTo "Database dispatcher queue is full or closed")
    | Ok _ ->
      Assert.Fail
        "Dispatch should fail with controlled error after dispatcher has stopped"
