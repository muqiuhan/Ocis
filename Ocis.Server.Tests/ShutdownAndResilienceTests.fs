...
type ShutdownAndResilienceTests() =
    ...
    member _.NonTransientSocketErrorsAreNotClassifiedAsTransient(error: SocketError) =
        let ex = SocketException(int error) :> exn
    ...
    member _.RetryDelayGrowsExponentiallyAndCapsAtMaximum() =
        ...
        Assert.That(computeBoundedRetryDelayMs 0 baseDelayMs maxDelayMs, Is.EqualTo 25)
        Assert.That(computeBoundedRetryDelayMs 1 baseDelayMs maxDelayMs, Is.EqualTo 50)
        Assert.That(computeBoundedRetryDelayMs 2 baseDelayMs maxDelayMs, Is.EqualTo 100)
        Assert.That(computeBoundedRetryDelayMs 3 baseDelayMs maxDelayMs, Is.EqualTo 200)
        Assert.That(computeBoundedRetryDelayMs 4 baseDelayMs maxDelayMs, Is.EqualTo 200)
