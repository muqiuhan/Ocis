...
type TelemetrySmokeTests() =
    let testDbDir = Path.Combine(Path.GetTempPath(), $"ocis_telemetry_tests_{Guid.NewGuid():N}")
    ...
    let collectMeasurements (operation: unit -> unit) =
        ...
        listener.InstrumentPublished <- Action<Instrument, MeterListener>(fun instrument listener ->
            if instrument.Meter.Name = MeterName then
                listener.EnableMeasurementEvents instrument)

        listener.SetMeasurementEventCallback<int64>(fun instrument measurement _ _ ->
    ...
    member _.SuccessfulRequestEmitsTotalAndDurationMetrics() =
        ...
        let metrics =
            collectMeasurements (fun () ->
                let request = validSetRequest (toBytes "ok-key") (toBytes "ok-value")
                let response = RequestHandler.ProcessValidRequest dispatcher request |> Async.RunSynchronously
