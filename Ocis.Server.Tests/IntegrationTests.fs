...
type IntegrationTests() =
  ...
  let testDbDir =
    Path.Combine(Path.GetTempPath(), "ocis_test_integration")
  ...
  member this.TestInvalidDurabilityMode() =
    ...
    | Error _ ->
      Assert.Pass "Invalid durability mode correctly failed validation"
  ...
  member this.TestInvalidGroupCommitWindowMs() =
    ...
    let badConfig =
      {invalidConfig with
          GroupCommitWindowMs = 0}

    match ConfigHelper.ValidateConfig badConfig with
    | Ok() -> Assert.Fail "Invalid group commit window should fail validation"
    | Error _ ->
      Assert.Pass "Invalid group commit window correctly failed validation"
  ...
  member this.TestInvalidGroupCommitBatchSizeZero() =
    ...
    let badConfig =
      {invalidConfig with
          GroupCommitBatchSize = 0}

    match ConfigHelper.ValidateConfig badConfig with
    | Ok() ->
      Assert.Fail "Invalid group commit batch size should fail validation"
    | Error msg ->
      Assert.That(msg, Is.EqualTo "GroupCommitBatchSize must be greater than 0")
  ...
  member this.TestInvalidDbQueueCapacityZero() =
    ...
    | Error msg ->
      Assert.That(msg, Is.EqualTo "DbQueueCapacity must be greater than 0")
  ...
  member this.TestInvalidCheckpointMinIntervalMsZero() =
    ...
    match ConfigHelper.ValidateConfig badConfig with
    | Ok() ->
      Assert.Fail "Invalid checkpoint minimum interval should fail validation"
    | Error msg ->
      Assert.That(
        msg,
        Is.EqualTo "CheckpointMinIntervalMs must be greater than 0"
      )
  ...
  member this.TestInvalidCheckpointMinIntervalMsNegative() =
    ...
    let badConfig =
      {invalidConfig with
          CheckpointMinIntervalMs = -100}

    match ConfigHelper.ValidateConfig badConfig with
    | Ok() ->
      Assert.Fail "Negative checkpoint minimum interval should fail validation"
    | Error msg ->
      Assert.That(
        msg,
        Is.EqualTo "CheckpointMinIntervalMs must be greater than 0"
  ...
  member this.TestInvalidLogLevel() =
    ...
    | Error msg ->
      Assert.That(
        msg,
        Is.EqualTo "LogLevel must be one of: Debug, Info, Warn, Error, Fatal"
      )
  ...
  member this.TestProgramLogLevelOptionValidationRejectsInvalid() =
    ...
    | Error msg ->
      Assert.That(
        msg,
        Is.EqualTo "Log level must be one of: Debug, Info, Warn, Error, Fatal"
      )
    ...
    use host =
      Host
        ...
        .ConfigureServices(fun _ services ->
          services.AddSingleton<IOptions<Ocis.Server.Host.OcisServerOptions>>(
            Options.Create(
              Ocis.Server.Host.OcisServerOptions.FromConfig config
            )
          )
          |> ignore

          services.AddHostedService<Ocis.Server.Host.OcisHostedService>()
          |> ignore)
        .Build()
  ...
  member this.TestHostStartupFailsWhenPortAlreadyInUse() =
    ...
    let ex =
      Assert.Throws<SocketException>(fun () ->
