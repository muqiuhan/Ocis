...
type OcisError =
  ...
  override this.ToString () : string =
    ...
    | KeyNotFound key ->
      $"Key not found: {System.Text.Encoding.UTF8.GetString key}"
    ...
    | InvalidSSTableLevel level ->
      $"Invalid SSTable level: {level} (must be >= 0)"
    | SSTableFlushError (path, reason) ->
      $"Failed to flush SSTable to '{path}': {reason}"
    | SSTableRecordCountMismatch (expected, actual) ->
      $"SSTable record count mismatch: expected {expected}, got {actual}"
    | SSTableRecordIndexOutOfRange (index, count) ->
      $"SSTable record index {index} out of range (count: {count})"
    | IOError (operation, path, message) ->
      $"I/O error during {operation} at '{path}': {message}"
    | UnauthorizedAccess path -> $"Access denied: {path}"
    | SerializationError message -> $"Serialization error: {message}"
    | ResourceDisposalError (resource, message) ->
      $"Failed to dispose {resource}: {message}"
...
type ResultBuilder () =
  ...
  member _.TryFinally
    (m : unit -> Result<'T, 'E>, compensation : unit -> unit)
    =
    try
      m ()
    finally
