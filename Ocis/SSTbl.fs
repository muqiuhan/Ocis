...
type SSTbl
    ...
    interface IEnumerable<KeyValuePair<byte array, ValueLocation>> with
        member this.GetEnumerator () =
            ...
            let readKeyValuePair (stream : FileStream, reader : BinaryReader, offset : int64) : KeyValuePair<byte array, ValueLocation> =
                lock stream (fun () ->
    ...
    static member Open (path : string) : SSTbl option =
        ...
        try
            ...
            fileStream.Seek (-(footerSize + 4 |> int64), SeekOrigin.End)
            |> ignore
