...
type Valog (path : string, fileStream : FileStream, reader : BinaryReader, writer : BinaryWriter, head : int64, tail : int64) =
    ...
    static member Create (path : string) : Result<Valog, string> =
        try
            ...
            let fileStream = new FileStream (path, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite)
    ...
    static member CollectGarbage (valog : Valog, liveLocations : Set<int64>) : Async<Result<Valog * Map<int64, int64>, string> option> = async {
        ...
        try
            ...
            use newFileStream = new FileStream (tempValogPath, FileMode.Create, FileAccess.Write, FileShare.None)
            ...
            use oldFileStream = new FileStream (valog.Path, FileMode.Open, FileAccess.Read, FileShare.Read)
