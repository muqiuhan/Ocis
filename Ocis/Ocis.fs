...
type OcisDB
  ...
  static member private calculateCompactionPriority
    ...
    if level = 0 then
      ...
      | Some nextSSTables ->
        ...
        if overlapSize > 0L && sizeRatio > 0.8 then
          High
        elif overlapSize > 0L then
          Medium
        else
          Low
      | None ->
  ...
  member this.UpdateSSTablesAfterCompaction
    ...
    | Some newSSTbl ->
      Logger.Debug
        $"Compaction successful: Merged {sstblsToRemoveFromSource.Length
                                         + sstblsToRemoveFromTarget.Length} SSTables to Level {newSSTbl.Level}. New SSTable: {newSSTbl.Path}"
    | None ->
      Logger.Debug
        $"Compaction successful: Removed {sstblsToRemoveFromSource.Length
                                          + sstblsToRemoveFromTarget.Length} SSTables (all deletion markers or no new SSTbl)."
      ...
      try
        ...
        if liveLocations.IsEmpty then
          ...
          | Ok(newValog, remappedLocations) ->
            ...
            Logger.Info
              $"GC completed: {remappedLocations.Count} locations remapped"
  ...
  static member Open
        ...
        | Ok parsedDurabilityMode ->
          ...
          let createDirResult =
            ...
            | :? System.IO.IOException as ioEx ->
              Error
                $"Failed to create database directory '{dir}': {ioEx.Message}"
          ...
          | Ok() ->
            ...
            |> Result.bind(fun valog ->
              ...
              |> Result.bind(fun wal ->
                ...
                Logger.Info
                  $"Replayed {replayCount} WAL entries into MemTable"
    ...
    | :? System.IO.IOException as ioEx when
      ioEx.Message.Contains "disk"
      || ioEx.Message.Contains "full"
      ->
