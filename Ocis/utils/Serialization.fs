...
module internal Serialization =
    ...
    if len = 0 then
      ...
      try
        ...
        while totalRead < len do
          ...
          if read = 0 then
            failwith
              $"Unexpected end of stream: expected {len} bytes, got {totalRead}"
  ...
  let inline readByteArrayIntoBuffer
    ...
    if len > buffer.Length then
      failwith
        $"Buffer too small: required {len} bytes, buffer size {buffer.Length}"
    ...
    if len = 0 then
      ...
      try
        ...
        while totalRead < len do
          ...
          if read = 0 then
            failwith
              $"Unexpected end of stream: expected {len} bytes for key, got {totalRead}"
