...
type OcisDBBenchmarks() =
  ...
  member this.GlobalSetup() =
    ...
    keys <-
      Array.init this.Count (fun i -> Encoding.UTF8.GetBytes $"perf_key_{i}")

    values <-
      Array.init this.Count (fun i ->
        Encoding.UTF8.GetBytes($"perf_value_{i}_" + new string('A', 100)))
