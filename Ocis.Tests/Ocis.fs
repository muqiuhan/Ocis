...
type OcisDBTests () =
    ...
    member this.Set_ShouldTriggerMemtableFlush () =
        ...
        | Ok db ->
            ...
            match getResult with
            | Ok (Some actualValue) -> Assert.That (Encoding.UTF8.GetString actualValue, Is.EqualTo (Encoding.UTF8.GetString valueToRetrieve))
    ...
    member this.Close_ShouldDisposeAllResources () =
        ...
        | Ok db ->
            ...
            Assert.That (
                fun () ->
                    use walFs =
                        new FileStream (Path.Combine (testDbPath, "wal.log"), FileMode.Open, FileAccess.ReadWrite, FileShare.None)

                    use valogFs =
                        new FileStream (Path.Combine (testDbPath, "valog.vlog"), FileMode.Open, FileAccess.ReadWrite, FileShare.None)
