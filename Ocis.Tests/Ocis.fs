    ...
    member this.TearDown() =
        ...
        if Directory.Exists tempDir then
            ...
            while not deleted && currentRetry < maxRetries do
                ...
                | ex ->
                    printfn $"TearDown: Unexpected error deleting directory '{tempDir}': {ex.Message}"

                    reraise ()

            if not deleted then
                Assert.Fail $"TearDown: Failed to delete directory '{tempDir}' after {maxRetries} retries."
