...
type ErrorHandlingTests () =
  ...
  member this.SSTableFlush_ShouldHandleNullMemtbl () =
    ...
    | Error err ->
      Assert.That (
        err.ToString (),
        Does.Contain "null",
        "Error should indicate null memtbl"
      )
  ...
  member this.SSTableFlush_ShouldHandlePathTooLong () =
    ...
    let longPath =
      Path.Combine (tempDir, String ('x', 500) + ".sst")
    ...
    | Error err ->
      ...
      Assert.That (
        err.ToString (),
        Does.Contain "error",
        "Error should indicate I/O or path issue"
      )
  ...
  member this.SSTableFlush_ShouldHandleDirectoryNotFound () =
    ...
    | Error err ->
      Assert.That (
        err.ToString (),
        Does.Contain "error",
        "Error should indicate I/O issue"
      )
  ...
  member this.OcisDBOpen_ShouldHandleCorruptedSSTable () =
    ...
    | Ok db ->
      ...
      use fs =
        new FileStream (sstPath, FileMode.Open, FileAccess.Write)
  ...
  member this.FileSystemStressTest_ShouldHandleManyFiles () =
    ...
    | Ok db ->
      ...
      let sstableCount =
        db.SSTables
        |> Map.toSeq
        |> Seq.sumBy (fun (_, lst) -> lst.Length)
