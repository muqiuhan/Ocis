...
type WalTests () =
    ...
    member this.``Create_ShouldOpenFileAndAppendToExistingFile`` () =
        ...
        | Ok wal ->
            ...
            Assert.That (wal.FileStream.Position, Is.EqualTo (0L), "File pointer should be at the beginning of the file (for a new file).")
    ...
    member this.``Replay_ShouldHandleMultipleEntries`` () =
        ...
        | Ok wal ->
            ...
            let entries = [
                ...
                WalEntry.Set (Encoding.UTF8.GetBytes ("key3"), 30L)
            ] // Use int64 literal directly
    ...
    member this.``Flush_ShouldEnsureDataIsPersisted`` () =
        ...
        | Ok wal ->
            ...
            Assert.That (replayedEntries.Length, Is.EqualTo (1), "Should be able to replay persisted data after reopening.")
