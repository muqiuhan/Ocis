...
type AdvancedBenchmarks () =
  ...
  member this.GlobalSetup () =
    ...
    hotValues <-
      Array.init
        hotDataCount
        (fun i -> Encoding.UTF8.GetBytes ($"hot_value_{i}" + String ('H', 50)))
    ...
    sequentialKeys <-
      Array.init
        (this.DataSize / 4)
        (fun i -> Encoding.UTF8.GetBytes $"seq_{i:D10}")
  ...
  member this.LayeredWrite () =
    ...
    if sequentialKeys.Length > 0 then
      for i = 0 to sequentialKeys.Length - 1 do
        db.Set (sequentialKeys[i], sequentialValues[i])
        |> ignore
  ...
  member this.RealisticWorkload () =
    ...
    for i = 0 to 999 do
      if random.NextDouble () < 0.7 then
        ...
        let key =
          if
            random.NextDouble () < this.HotDataRatio
            && hotKeys.Length > 0
          then
