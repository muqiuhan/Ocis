...
type ValogTests () =
    ...
    member this.``Read_ShouldReturnNoneForInvalidLocation`` () =
        ...
        | Ok valog ->
            ...
            Assert.That (valog.Read(invalidOffsetBefore).IsNone, Is.True, "Reading an invalid offset before Head should return None.")

            Assert.That (valog.Read(invalidOffsetAfter).IsNone, Is.True, "Reading an invalid offset after Head should return None.")
