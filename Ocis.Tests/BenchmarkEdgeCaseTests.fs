...
type BenchmarkEdgeCaseTests () =
  ...
  member this.RealisticWorkload_ShouldHandleEmptyOperationsArray () =
    ...
    | Ok db ->
      ...
      async {
        let operations = Array.zeroCreate<Async<unit>> 0 // Empty array

        for i = 0 to operations.Length - 1 do
          () // No operations to perform

        // Execute empty operations array
  ...
  member this.CrossSSTableRead_ShouldHandleEmptyReadTasks () =
    ...
    | Ok db ->
      ...
      async {
        let readTasks = [] // Empty read tasks list

        // Execute empty read tasks
  ...
  member this.BenchmarkDataGeneration_ShouldHandleExtremeRatios () =
    ...
    | Ok db ->
      ...
      for i = 0 to min 5 (hotKeys.Length - 1) do
        let getResult = db.Get hotKeys[i] |> Async.RunSynchronously
  ...
  member this.LayeredWrite_ShouldCreateMultipleSSTables () =
    ...
    | Ok db ->
      ...
      for i = 0 to min 10 (coldKeys.Length - 1) do
        let getResult = db.Get coldKeys[i] |> Async.RunSynchronously
          ...
          [
            ...
            for i = 0 to 5 do
              yield
                async {
                  let! result =
                    db.Get (Encoding.UTF8.GetBytes $"nonexistent_{i}")
      ...
      async {
        ...
        for i = 0 to min 20 199 do
          ...
          | Ok (Some value) ->
            ...
            let actual = Encoding.UTF8.GetString value
            if actual <> expected then failwith $"Value mismatch for key {i}"
  ...
  member this.RealisticWorkload_ShouldHandleMixedReadWriteOperations () =
    ...
    | Ok db ->
      ...
      // Setup initial data
      let initialKeys =
        Array.init 50 (fun i -> Encoding.UTF8.GetBytes $"initial_{i}")
      ...
      let operations = Array.zeroCreate<Async<unit>> 100
