...
type SSTblTests () =
    ...
    member this.``Flush_ShouldCreateSSTableFileWithCorrectData`` () =
        ...
        for i = 0 to 4 do
            ...
            Assert.That (actualValue.Value, Is.EqualTo (expectedValue), $"Value for key {Encoding.UTF8.GetString (key)} should be correct.")
    ...
    member this.``Flush_ShouldHandleEmptyMemtbl`` () =
        ...
        Assert.That (sstbl.TryGet(Encoding.UTF8.GetBytes ("anykey")).IsNone, Is.True, "TryGet should return None for empty SSTable.")
    ...
    member this.``Seq_ShouldIterAllKeyValuePairsInOrder`` () =
        ...
        |> Seq.iter (fun (KeyValue (key, valueLocation)) ->
            ...
            Assert.That (valueLocation, Is.EqualTo (expectedValue), $"Enumerated value at index {expectedIdx} should match.")
    ...
    member this.``Dispose_ShouldCloseFileStream`` () =
        ...
        Assert.That (
            fun () ->
                use fs = new FileStream (testFilePath, FileMode.Open, FileAccess.ReadWrite, FileShare.None)
