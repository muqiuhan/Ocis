...
type ThreadAffinityTests() =
  ...
  member _.CrossThreadGet_ShouldThrowInvalidOperationException() =
    ...
    | Ok db ->
      ...
      | Some ex ->
        Assert.Fail
          $"Expected InvalidOperationException but got {ex.GetType().FullName}: {ex.Message}"
      | None ->
        Assert.Fail
          "Expected InvalidOperationException from cross-thread access"
  ...
  member _.SameThreadValogOperations_ShouldNotRaiseFalsePositives() =
    ...
    | Ok valog ->
      ...
      | None ->
        Assert.Fail "Expected Valog.ReadValueOnly to succeed on owner thread"
