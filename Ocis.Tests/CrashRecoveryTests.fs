...
type CrashRecoveryTests() =
    ...
    let tempRoot = Path.Combine(Path.GetTempPath(), "ocis_crash_recovery_tests")
    let mutable testDbPath = ""

    let openBalancedDb () : OcisDB =
        match OcisDB.Open(testDbPath, 1000, durabilityMode = "Balanced", groupCommitWindowMs = 10) with
        | Ok db -> db
    ...
    let valueOf (db: OcisDB) (key: string) : string option =
        ...
        | Ok None -> None
        | Error msg -> Assert.Fail $"Get failed for key '{key}': {msg}"; None

    let setValue (db: OcisDB) (key: string) (value: string) =
        let result = db.Set(Encoding.UTF8.GetBytes key, Encoding.UTF8.GetBytes value)
    ...
    member _.BalancedMode_NoCorruption_AfterRepeatedOpenCloseCycles() =
        ...
        for i = 1 to 5 do
            ...
            Assert.That(valueOf finalReopen key, Is.EqualTo expected, $"Corruption detected at key '{key}'")
