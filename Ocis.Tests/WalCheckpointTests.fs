...
type WalCheckpointTests() =
    ...
    let tempRoot = Path.Combine(Path.GetTempPath(), "ocis_wal_checkpoint_tests")
    ...
    let getValueAsString (db: OcisDB) (key: string) : string option =
        ...
        | Ok None -> None
        | Error msg -> Assert.Fail $"Get failed for '{key}': {msg}"; None
    ...
    member _.Wal_IsResetAfterCheckpointTriggeringFlush() =
        ...
        db.Set(Encoding.UTF8.GetBytes "k1", Encoding.UTF8.GetBytes "v1") |> ignore
        let sizeBeforeFlush = (FileInfo walPath).Length

        db.Set(Encoding.UTF8.GetBytes "k2", Encoding.UTF8.GetBytes "v2") |> ignore
        db.Set(Encoding.UTF8.GetBytes "k3", Encoding.UTF8.GetBytes "v3") |> ignore

        let sizeAfterFlush = (FileInfo walPath).Length

        Assert.That(sizeBeforeFlush, Is.GreaterThan 0L)
        Assert.That(sizeAfterFlush, Is.EqualTo 0L, "WAL should be reset after successful checkpoint")
    ...
    member _.Recovery_RemainsCorrectAfterCheckpointReset() =
        do
            ...
            db.Set(Encoding.UTF8.GetBytes "a", Encoding.UTF8.GetBytes "1") |> ignore
            db.Set(Encoding.UTF8.GetBytes "b", Encoding.UTF8.GetBytes "2") |> ignore
            db.Set(Encoding.UTF8.GetBytes "c", Encoding.UTF8.GetBytes "3") |> ignore

            db.Set(Encoding.UTF8.GetBytes "tail", Encoding.UTF8.GetBytes "tail-value") |> ignore

        use reopened =
            match OcisDB.Open(testDbPath, 3) with
            | Ok opened -> opened
            | Error msg -> Assert.Fail $"Failed to reopen DB: {msg}"; Unchecked.defaultof<OcisDB>
        ...
        Assert.That(getValueAsString reopened "tail", Is.EqualTo(Some "tail-value"))
    ...
    member _.MultipleCheckpointCycles_DoNotCorruptState() =
        do
            ...
            db.Set(Encoding.UTF8.GetBytes "a", Encoding.UTF8.GetBytes "1") |> ignore
            db.Set(Encoding.UTF8.GetBytes "b", Encoding.UTF8.GetBytes "1") |> ignore

            db.Set(Encoding.UTF8.GetBytes "a", Encoding.UTF8.GetBytes "2") |> ignore
            db.Set(Encoding.UTF8.GetBytes "c", Encoding.UTF8.GetBytes "3") |> ignore

            db.Delete(Encoding.UTF8.GetBytes "b") |> ignore
            db.Set(Encoding.UTF8.GetBytes "d", Encoding.UTF8.GetBytes "4") |> ignore

            db.Set(Encoding.UTF8.GetBytes "tail", Encoding.UTF8.GetBytes "5") |> ignore
    ...
    member _.Delete_AfterCheckpoint_RemainsDeletedAfterReopen_StrictMode() =
        do
            ...
            let firstSet = db.Set(Encoding.UTF8.GetBytes "target", Encoding.UTF8.GetBytes "value-1")
            Assert.That(firstSet.IsOk, Is.True)

            let secondSet = db.Set(Encoding.UTF8.GetBytes "other", Encoding.UTF8.GetBytes "value-2")
